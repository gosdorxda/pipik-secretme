...
export const referralsTable = pgTable("referrals", {
  ...
  referredUserId: text("referred_user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  pointsAwarded: integer("points_awarded").notNull().default(10),
  upgradeBonusAwarded: boolean("upgrade_bonus_awarded").notNull().default(false),
