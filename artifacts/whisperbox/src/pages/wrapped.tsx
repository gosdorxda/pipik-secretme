  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center rounded-md z-10"
      ...
      <div className="text-center px-6">
        ...
        <p className="text-lg font-bold mb-1" style={{ color: W_TEXT }}>Fitur Premium</p>
        <p className="text-sm mb-5" style={{ color: W_MUTED }}>
          Upgrade ke Premium untuk membuka sisa Wrapped-mu.
        </p>
        <Link href="/upgrade">
          <Button style={{ background: W_MINT, color: "#1a443c" }} className="font-bold">
            Upgrade Sekarang
          </Button>
        </Link>
      </div>
...
function BigNumber({ value, label, sublabel }: { value: number; label: string; sublabel?: string }) {
  return (
    <div className="text-center">
      <p className="text-sm font-semibold uppercase tracking-widest mb-4" style={{ color: W_MINT_DIM }}>{label}</p>
      <p className="font-bold leading-none mb-3" style={{ fontSize: "clamp(5rem, 20vw, 9rem)", color: W_MINT }}>
        {value.toLocaleString("id-ID")}
      </p>
      {sublabel && <p className="text-xl font-medium" style={{ color: W_TEXT }}>{sublabel}</p>}
    </div>
  );
}
...
function Slide0Intro({ displayName, year, month }: { displayName: string; year: number; month: number | null }) {
  const period = month ? `${MONTHS_ID[month - 1]} ${year}` : `Tahun ${year}`;
  return (
    <SlideShell>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full opacity-10"
             style={{ background: `radial-gradient(circle, ${W_MINT} 0%, transparent 70%)` }} />
      </div>
      <div className="relative text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-8"
             style={{ background: `${W_MINT}20`, color: W_MINT, border: `1px solid ${W_MINT}40` }}>
          <Sparkles className="w-3.5 h-3.5" /> WhisperBox Wrapped
        </div>
        <h1 className="text-4xl md:text-6xl font-bold mb-4 leading-tight" style={{ color: W_TEXT }}>
...
function Slide1TotalMessages({ data, active }: { data: WrappedData; active: boolean }) {
...
function Slide2BusiestDay({ data, active }: { data: WrappedData; active: boolean }) {
  ...
  return (
    <SlideShell>
      <p className="text-sm font-semibold uppercase tracking-widest mb-6" style={{ color: W_MINT_DIM }}>Hari Tersibuk</p>
      <p className="text-5xl md:text-7xl font-bold mb-2" style={{ color: W_MINT }}>
        {DAYS_ID[peakDay.day]}
      </p>
...
function Slide3PeakHour({ data, active }: { data: WrappedData; active: boolean }) {
  ...
  return (
    <SlideShell>
      <p className="text-sm font-semibold uppercase tracking-widest mb-6" style={{ color: W_MINT_DIM }}>Jam Tersibuk</p>
      <div className="text-center mb-8">
        <p className="text-7xl mb-3">{emoji}</p>
        <p className="text-5xl md:text-6xl font-bold mb-2" style={{ color: W_MINT }}>
...
function Slide4LongestMessage({ data, isPremium }: { data: WrappedData; isPremium: boolean }) {
  const msg = data.longestMessage;
  return (
    <SlideShell style={{ position: "relative" }}>
      <PremiumLock isPremium={isPremium} />
      <p className="text-sm font-semibold uppercase tracking-widest mb-6" style={{ color: W_MINT_DIM }}>
        Pesan Terpanjang
      </p>
      {msg ? (
        <>
...
function Slide5TopWords({ data, isPremium }: { data: WrappedData; isPremium: boolean }) {
  ...
  return (
    <SlideShell style={{ position: "relative" }}>
      <PremiumLock isPremium={isPremium} />
      <p className="text-sm font-semibold uppercase tracking-widest mb-8" style={{ color: W_MINT_DIM }}>
        Kata yang Sering Kamu Dengar
      </p>
      {data.topWords.length > 0 ? (
        <div className="flex flex-col items-center gap-3">
          {data.topWords.map((w, i) => (
            <div key={w.word} className={`font-bold ${sizes[i]} leading-none`}
...
function Slide6Replies({ data, active, isPremium }: { data: WrappedData; active: boolean; isPremium: boolean }) {
  const countUp = useCountUp(data.repliedMessages, active);
  const pct = data.totalMessages > 0 ? Math.round((data.repliedMessages / data.totalMessages) * 100) : 0;
  return (
    <SlideShell style={{ position: "relative" }}>
      <PremiumLock isPremium={isPremium} />
      <p className="text-sm font-semibold uppercase tracking-widest mb-6" style={{ color: W_MINT_DIM }}>
      ...
      <p className="text-xl font-medium -mt-6 mb-6" style={{ color: W_TEXT }}>
        dari <span style={{ color: W_MINT }}>{data.totalMessages}</span> pesan yang kamu balas
      </p>
      <div className="w-full max-w-xs rounded-full overflow-hidden" style={{ height: 8, background: `${W_MINT}20` }}>
...
function Slide7FinalCard({
  ...
  const stats = [
    ...
    { label: "Pesan Dibalas", value: data.repliedMessages.toLocaleString("id-ID") },
  ...
  return (
    <SlideShell style={{ position: "relative" }}>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-px" style={{ background: `${W_MINT}10` }}>
        ...
        <div className="px-6 py-3 text-center">
          <p className="text-xs" style={{ color: W_MUTED }}>whisperbox — terima kejujuran, tanpa takut.</p>
        </div>
      </div>
        ...
        <div className="flex gap-2 w-full">
          ...
          >
            {downloadStatus === "generating" ? (
              <>
                <div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: `${W_MINT}40`, borderTopColor: W_MINT }} />
                Membuat…
              </>
            ) : downloadStatus === "done" ? (
          ...
          <Button
            ...
            style={{
              background: copyStatus === "done" ? "#22c55e" : copyStatus === "unsupported" ? "#ef4444" : W_CARD,
              color: copyStatus === "done" || copyStatus === "unsupported" ? "#fff" : W_TEXT,
              border: `1px solid ${W_MINT}40`,
...
export default function WrappedPage() {
  ...
  if (phase === "select") {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6"
        ...
        <Link href="/dashboard" className="absolute top-6 left-6 flex items-center gap-1.5 text-sm"
              style={{ color: W_MUTED }}>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-md p-6 space-y-4" style={{ background: W_CARD }}>
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: W_MUTED }}>
                Bulan <span style={{ color: W_MUTED, fontWeight: "normal" }}>(opsional — kosong = tahunan)</span>
              </label>
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: W_BG }}>
        <div className="w-10 h-10 rounded-full border-4 animate-spin" style={{ borderColor: `${W_MINT}40`, borderTopColor: W_MINT }} />
        <p className="text-sm" style={{ color: W_MUTED }}>Menyiapkan Wrapped-mu…</p>
      </div>
    );
  }
  ...
  if (!wrapped || wrapped.totalMessages === 0) {
    const period = month ? `${MONTHS_ID[month - 1]} ${year}` : `tahun ${year}`;
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center" style={{ background: W_BG }}>
        <p className="text-5xl mb-4">💬</p>
        <h2 className="text-2xl font-bold mb-2" style={{ color: W_TEXT }}>Belum Ada Pesan</h2>
        <p className="text-sm mb-6 max-w-xs" style={{ color: W_MUTED }}>
          Kamu belum menerima pesan di {period}. Bagikan linkmu dan mulai kumpulkan cerita!
        </p>
  ...
  const slides = [
    <Slide0Intro key="intro" displayName={displayName} year={year} month={month} />,
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: W_BG, color: W_TEXT }}>
      {/* Top bar */}
      <div className="flex items-center justify-between px-6 py-4 shrink-0">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: W_MINT }}>
      ...
      <div className="flex items-center justify-between px-6 py-5 shrink-0">
        ...
        <button
          onClick={slide === TOTAL_SLIDES - 1 ? () => setPhase("select") : goNext}
          className="w-10 h-10 rounded-full flex items-center justify-center transition-all"
          style={{ background: W_MINT, color: "#1a443c" }}
        >
