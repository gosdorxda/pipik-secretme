...
const FREE_FEATURES = [
  ...
  {
    label: "Username & display name kustom",
    icon: <Star className="w-3.5 h-3.5" />,
  },
  { label: "Bio profil singkat", icon: <Check className="w-3.5 h-3.5" /> },
  {
    label: "Terima pesan anonim tak terbatas",
    icon: <Check className="w-3.5 h-3.5" />,
  },
  { label: "Balas pesan", icon: <Check className="w-3.5 h-3.5" /> },
  {
    label: "Share message card ke IG/Twitter",
    icon: <Check className="w-3.5 h-3.5" />,
  },
  {
    label: "Profil publik dengan link personal",
    icon: <Check className="w-3.5 h-3.5" />,
  },
...
const PREMIUM_FEATURES = [
  ...
  {
    label: "Notifikasi email otomatis tiap pesan masuk",
    icon: <Mail className="w-3.5 h-3.5" />,
    highlight: true,
  },
  {
    label: "Notifikasi balasan ke pengirim anonim (opsional)",
    icon: <Bell className="w-3.5 h-3.5" />,
    highlight: true,
  },
  {
    label: "Badge ✓ Premium eksklusif di profil publik",
    icon: <Crown className="w-3.5 h-3.5" />,
    highlight: true,
  },
  {
    label: "Kampanye pesan: ajukan pertanyaan ke pengunjung profilmu",
    icon: <Zap className="w-3.5 h-3.5" />,
    highlight: false,
  },
  {
    label: "Akses Wrapped: ringkasan pesan tahunanmu",
    icon: <Sparkles className="w-3.5 h-3.5" />,
    highlight: false,
  },
  {
    label: "Poin referral bonus saat ajak teman upgrade",
    icon: <Gift className="w-3.5 h-3.5" />,
    highlight: false,
  },
  {
    label: "Prioritas dukungan & fitur baru",
...
const TRUST_POINTS = [
  {
    ...
    desc: "Diproses oleh Tripay, payment gateway terpercaya di Indonesia",
  },
  {
    icon: "✅",
    title: "Bayar Sekali",
    desc: "Tidak ada biaya bulanan atau biaya tersembunyi. Lifetime access.",
  },
  {
    icon: "⚡",
    title: "Aktif Instan",
    desc: "Premium langsung aktif setelah pembayaran dikonfirmasi",
  },
];
      ...
      onError: (err: any) => {
        ...
        if (msg === "already_premium") {
          toast({
            ...
            description: "Akun kamu sudah aktif Premium.",
          });
          setStep("success");
        } else {
          toast({
        ...
        <div className="max-w-md mx-auto py-12 flex flex-col items-center text-center gap-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Semua fitur premium sudah aktif. Sekarang kamu bisa menambahkan
              social links, notifikasi email, dan lebih banyak lagi.
            </p>
        ...
        <div className="max-w-md mx-auto py-8 space-y-5">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Gunakan aplikasi m-banking, GoPay, OVO, Dana, atau dompet digital
              lainnya
            </p>
          </div>
          ...
          <div className="border border-border bg-white rounded-md p-6 flex flex-col items-center gap-4 shadow-sm">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Pembayaran sekali, premium selamanya
              </p>
            </div>
            ...
            {qrString && timeLeft !== "expired" && (
              <div className="w-full">
                <p className="text-[10px] text-muted-foreground mb-1.5 text-center">
                  Data QR (untuk dompet digital yang mendukung paste)
                </p>
          ...
          <div className="border border-border rounded-md p-4">
            <p className="text-xs font-semibold text-foreground mb-3">
              Diterima oleh semua aplikasi QRIS:
            </p>
      ...
      <div className="space-y-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="border-2 border-primary rounded-md p-5 bg-primary/5 relative overflow-hidden">
            ...
            <p className="text-xs text-muted-foreground mb-5">
              sekali bayar &middot; aktif selamanya
            </p>
            <ul className="space-y-2.5">
              {PREMIUM_FEATURES.map((f) => (
                <li key={f.label} className="flex items-start gap-2.5 text-sm">
                  <span
                    className={`mt-0.5 shrink-0 ${f.highlight ? "text-primary-foreground" : "text-primary"}`}
                  >
                    {f.icon}
                  </span>
                  <span
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Bayar dari m-banking atau dompet digital mana saja
              </p>
            </div>
          </div>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {TRUST_POINTS.map((t) => (
            <div
              key={t.title}
              className="flex gap-3 p-4 border border-border rounded-md bg-white"
            >
        ...
        <div className="flex flex-col items-center gap-3 pb-4">
          ...
          >
            {createPayment.isPending ? (
              ...
              <>
                <QrCode className="w-4 h-4" /> Bayar dengan QRIS &mdash;{" "}
                {formatPrice(premiumPrice)}
              </>
            )}
          </Button>
          <p className="text-xs text-muted-foreground text-center max-w-xs leading-relaxed">
            Pembayaran diproses secara aman oleh <strong>Tripay</strong>. QR
            code berlaku 24 jam setelah dibuat. Tidak ada tagihan rutin.
          </p>
        </div>
      </div>
    </AppLayout>
